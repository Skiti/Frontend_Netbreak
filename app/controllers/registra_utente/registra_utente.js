...
.controller('registra_utente_ctrl', function($scope, $http, $window, $location, ServicesHost) {
  ...
  $scope.submitForm = function() {
        ...
        if( $scope.ok == true ) {
			...
			$http.post(ServicesHost.users()+"basicclient_registration?"
    ...
    $scope.uploadavatar = function(element) {
		...
		reader.onload = function(event) {
			...
			var uri = $http({
				method  : 'POST',
				url     : ServicesHost.filehandler()+'setFile?'+'extension='+extension, 
				...
				$scope.avataruri = ServicesHost.local()+'resources/uploaded_images/'+response.data.$;
           });
       }
