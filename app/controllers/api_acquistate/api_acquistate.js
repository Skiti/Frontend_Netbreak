...
.controller('api_acquistate_ctrl', function($scope, $http, ServicesHost) {
	...
	$http.get(ServicesHost.transactions()+"retrieve_mslist_from_clientid?Id="+localStorage.getItem("IdClient")).then(function(response) {
		...
		for(var i=0; i<$scope.servicesid.length; i++) {
			...
			$http.post(ServicesHost.microservices()+"retrieve_ms_info?Id="+$scope.servicesid[i].IdMS).then(function(response) {
				if(response.data) {
	...
	$scope.getNewAPIKey = function(oldapikey,idms) {
		...
		$http.post(ServicesHost.transactions()+"apikey_update?OldAPIKey="+oldapikey+"&NewAPIKey="+newapikey).then(function(response) {
