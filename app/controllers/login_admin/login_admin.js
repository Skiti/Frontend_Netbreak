...
.controller('login_admin_ctrl', function($scope, $http, $location, ServicesHost) {
	...
	$scope.login = function() {
		...
		if ($scope.email == null || $scope.password == null) {
			...
			console.log(passmd5);
		   	...
		   	$http.post(ServicesHost.users()+"admin_exists?Email="+$scope.email+"&Password="+passmd5
	   		).then(function(response) {
	        	if (response.data.$ == false) {
	        	    $scope.errors.push("Informazioni di autenticazione errate");
	        	    $scope.ok = false;
	        	} else {
	        		$http.post(ServicesHost.users()+"retrieve_admin_info_from_email?Email="+$scope.email).then(function(response) {
						// dati della session
	   					localStorage.setItem("Session", "Admin");
						localStorage.setItem("IdAdmin", response.data.IdAdmin);
						localStorage.setItem("Name", response.data.Name);
						localStorage.setItem("Surname", response.data.Surname);
						localStorage.setItem("Avatar", ServicesHost.local()+"resources/icons/netbreaklogo.png");
