...
.controller('cambio_password_ctrl', function($scope, $http, $location, ServicesHost) {
	...
	$scope.changePassword = function() {
		...
		if( localStorage.getItem("Session") != 'true' ) {
			...
			if( $scope.OldPassword != $scope.RePassword ) {
				$scope.errors.push("Le password non coincidono"); 
				$scope.ok = false;
			}
			if( !checkPassword($scope.NewPassword) ) {
				$scope.errors.push("La nuova password non rispetta il formato richiesto"); 
				$scope.ok = false;
			}
			...
			var oldpassmd5 = (MD5($scope.OldPassword));
			
			if( oldpassmd5 != $scope.Password ) {
				$scope.errors.push("Password attuale errata"); 
				$scope.ok = false;
			}
			
			var newpassmd5 = (MD5($scope.NewPassword));
			
			if( $scope.ok ) {
		
				$http.post(ServicesHost.users()+"client_password_change?" +
