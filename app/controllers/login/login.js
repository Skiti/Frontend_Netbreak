...
.controller('login_ctrl', function($scope, $http, $location, ServicesHost) {
	...
	$scope.login = function() {
		...
		if( localStorage.getItem("Session") == 'true' ) {
			...
			var passmd5 = (MD5($scope.password));
		   	...
		   	$http.post(ServicesHost.users()+"user_exists?Email="+$scope.email+"&Password="+passmd5
	   		).then(function(response) {
	        	if (response.data.$ == false) {
	        	    $scope.errors.push("Informazioni di autenticazione errate");
	        	    $scope.ok = false;
	        	} else {
	        		$http.post(ServicesHost.users()+"retrieve_client_info_from_email?Email="+$scope.email).then(function(response) {
						// dati della session
