...
.controller('diventa_sviluppatore_ctrl', function($scope, $http, $location, ServicesHost) {
	...
	$scope.developerUpgrade = function() {
		...
		if( localStorage.getItem("ClientType") == 2 ) {
			...
			$http.post(ServicesHost.users()+"developer_upgrade?Id="+localStorage.getItem("IdClient")).then(function(response) {
