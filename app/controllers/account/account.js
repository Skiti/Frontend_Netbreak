...
.controller('account_ctrl', function($scope, $http, $location, $window, ServicesHost) {
	...
	$scope.updateProfile = function() {
		...
		if( localStorage.getItem("Session") != 'true' ) {
			...
			if( $scope.ok ) {
				...
				$http.post(ServicesHost.users()+"client_update?" +
    ...
    $scope.uploadavatar = function(element) {
		...
		reader.onload = function(event) {
			...
			var uri = $http({
				...
				$scope.Avatar = ServicesHost.local()+'resources/uploaded_images/'+response.data.$;
           });
       }
