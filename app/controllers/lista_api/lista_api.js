...
.controller('lista_api_ctrl', function($scope, $http, $routeParams, ServicesHost) {
	...
	$http.post(ServicesHost.microservices()+"homepage_ms_list").then(function(response) {
		...
		for(var i=0; i<$scope.services.length; i++) {
			...
			$http.post(ServicesHost.users()+"retrieve_client_anagraphics?Id="+$scope.services[i].Developer).then(function(response) {
	...
	$http.post(ServicesHost.users()+"retrieve_all_devid").then(function(response) {
