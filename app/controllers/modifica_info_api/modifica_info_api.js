...
.controller('modifica_info_api_ctrl', function($scope, $http, $routeParams, ServicesHost) {
	...
	$http.post(ServicesHost.microservices()+"retrieve_categories_of_ms?Id="+$scope.IdMS).then(function(response) {
	...
	$http.post(ServicesHost.microservices()+"retrieve_category_list").then(function(response) {
    ...
    $http.get(ServicesHost.microservices()+"retrieve_ms_info?Id="+$routeParams.api_id).then(function(response) {
		...
		if(localStorage.getItem("IdClient") == $scope.IdDeveloper && localStorage.getItem("Session") == 'true') {
			for(var i=0; i<$scope.categories.length; i++) {
				...
				if($scope.categories[i].Class == "label label-success") {
					...
					if(!found) {
						$http.post(ServicesHost.microservices()+"add_category_to_ms?IdMS="+$scope.IdMS+"&IdCategory="+$scope.categories[i].IdCategory);
					}
				}
				else {
					for(var j=0; j<$scope.actualcategories.length && !found; j++) {
					...
					if(found) {
						$http.post(ServicesHost.microservices()+"remove_category_from_ms?IdMS="+$scope.IdMS+"&IdCategory="+$scope.categories[i].IdCategory);
					}
				}
			}
	...
	$scope.uploadpdf = function(element) {
		...
		reader.onload = function(event) {
			...
			$http({
				method  : 'POST',
				url     : ServicesHost.filehandler()+'setFile?'+'extension='+extension, // location+operation Jolie
				...
				$scope.pdf_uri = ServicesHost.local()+'resources/api_pdf/'+response.data.$;
			});
		}
	...
	$scope.uploadlogo = function(element) {
		...
		reader.onload = function(event) {
			...
			$http({
				...
				// ritorna l'uri del file ottenuto dalla response di Jolie
				$scope.logo_uri = ServicesHost.local()+'resources/uploaded_images/'+response.data.$;
			});
		}
		// legge l'immagine come URL
		reader.readAsDataURL(element.files[0]);
	...
	$scope.submit = function() {
		...
		// controlla se l'utente sia il developer dell'API
		if(localStorage.getItem("IdClient") == $scope.IdDeveloper && localStorage.getItem("Session") == 'true') {
			$http.post(ServicesHost.microservices()+"microservice_update?" +
				"IdMS=" + $scope.IdMS +
				"&Name=" + $scope.nomeapi +
