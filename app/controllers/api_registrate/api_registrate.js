...
.controller('api_registrate_ctrl', function($scope, $http, $rootScope, ServicesHost) {
	...
	$http.post(ServicesHost.microservices()+"retrieve_ms_from_developerid?Id="+localStorage.getItem("IdClient")).then(function(response) {
		...
		for(var i=0; i<$scope.services.length; i++) {
			...
			$http.post(ServicesHost.transactions()+"retrieve_active_apikey_number_from_msid?Id="+$scope.services[i].IdMS).then(function(response) {
				if(response.data) {
					$scope.activelicenses[response.data.IdMS] = response.data.Licenses;
				}
			});
			$http.post(ServicesHost.transactions()+"retrieve_monthly_purchases_sum_from_msid?Id="+$scope.services[i].IdMS).then(function(response) {
				if(response.data) {
					$scope.monthprofits[response.data.IdMS] = (response.data.Sum * $scope.devprofits[response.data.IdMS]) / 100;
				}
			});
			$http.post(ServicesHost.transactions()+"retrieve_purchases_sum_from_msid?Id="+$scope.services[i].IdMS).then(function(response) {
				if(response.data) {
