...
.controller('api_ctrl', function($scope, $http, $routeParams, $location, ServicesHost) {
	...
	$http.post(ServicesHost.users()+"retrieve_client_info?Id="+localStorage.getItem("IdClient")).then(function(response) {
		...
		$http.post(ServicesHost.microservices()+"retrieve_ms_info?Id="+$routeParams.api_id).then(function(response) {
			...
			$http.post(ServicesHost.users()+"retrieve_client_anagraphics?Id="+$scope.IdDeveloper).then(function(response) {
			...
			$http.post(ServicesHost.transactions()+"retrieve_active_apikey_number_from_msid?Id="+$scope.IdMS).then(function(response) {
	...
	$http.post(ServicesHost.microservices()+"retrieve_categories_of_ms?Id="+$routeParams.api_id).then(function(response) {
	...
	$scope.changeIsActive = function() {
		...
		$http.post(ServicesHost.microservices()+"change_isactive?" +
			"IdMS=" + $scope.IdMS +
	...
	$http.post(ServicesHost.transactions()+"check_apikey_isactive?" +
	...
	$http.post(ServicesHost.microservices()+"retrieve_client_interface_from_id?Id="+$routeParams.api_id).then(function(response) {
	...
	$scope.buyAPI = function() {
		...
		if(canBuy()) {
			...
			$http.post(ServicesHost.users()+"credits_update?IdClient=" + localStorage.getItem("IdClient") + "&Credits=-" + $scope.Profit).then(function(response) {
				
				$http.post(ServicesHost.transactions()+"retrieve_apikey_from_msidandclient?IdMS="+$scope.IdMS+"&IdClient="+localStorage.getItem("IdClient")).then(function(response) {
					...
					if(response.data.$ == null) {
						...
						$http.post(ServicesHost.transactions()+"apikey_registration?" +
							"APIKey=" + $scope.APIKey +
							"&IdMS=" + $scope.IdMS +
							"&IdClient=" + localStorage.getItem("IdClient") + 
							...
							$http.post(ServicesHost.transactions()+"purchase_registration?" +
					...
					// nel caso di apikey già attiva o con remaining 0
						...
						$http.post(ServicesHost.transactions()+"apikey_remaining_update?"+"APIKey="+$scope.APIKey +"&Number="+$scope.Uses);
