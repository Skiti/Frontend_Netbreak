...
.controller('gestione_utenti_ctrl', function($scope, $http, $mdDialog, $route, ServicesHost) {
	...
	$http.post(ServicesHost.users()+"retrieve_all_client_info").then(function(response) {
		...
		angular.forEach($scope.users, function(value, key){
			$http.post(ServicesHost.microservices()+"retrieve_active_msnumber_from_devid?Id=" + value.IdClient).then(function(response) {
				value.Active_Ms_Number = response.data.$;
			});
		});

		angular.forEach($scope.users, function(value, key){
			$http.post(ServicesHost.microservices()+"retrieve_msnumber_from_devid?Id=" + value.IdClient).then(function(response) {
    ...
    $scope.showConfirm = function(ev, IdClient, numActive) {
		...
		if(numActive > 0){
			...
			$mdDialog.show(confirm).then(function() {
				$http.post(ServicesHost.users()+"client_delete?IdClient=" + IdClient +
