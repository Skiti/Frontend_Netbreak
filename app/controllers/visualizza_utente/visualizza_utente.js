...
.controller('visualizza_utente_ctrl', function($scope, $http, $routeParams, ServicesHost) {
	
	$http.post(ServicesHost.users()+"retrieve_client_info?Id="+$routeParams.user_id).then(function(response) {
		...
		$http.post(ServicesHost.users()+"retrieve_clienttype_info?Id="+response.data.ClientType).then(function(response) {
	...
	$http.post(ServicesHost.microservices()+"retrieve_ms_from_developerid?Id="+$routeParams.user_id).then(function(response) {
